...
async function handleMenu(primaryLayer, stage, newState) {
  const particleLayer = stage.findOne("#game-layer-1");

  await document.fonts.ready;
  await document.fonts.load('30px "Space Mono"');
  await document.fonts.load('70px "DynaPuff"');
...
function handleBounds(note, stage) {
  ...
  if (
    (spawnX < 0 && note.x() > stage.width()) ||
    (spawnY < 0 && note.y() > stage.height())
  ) {
    return true;
  } else if (
    (spawnX > stage.width() && note.x() + note.width() < 0) ||
    (spawnY > stage.height() && note.y() + note.height() < 0)
  ) {
    return true;
  }
  return false;
